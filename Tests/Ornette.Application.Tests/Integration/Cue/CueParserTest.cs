...
namespace Ornette.Application.Tests.Integration.Cue
{
    ...
    {
        private readonly CueParser _CueParser = new CueParser();
        private readonly List<string[]> _CueContents;

        public CueParserTest()
        {
            _CueParser = new CueParser();
            _CueContents = new[] { "Andrew Hill - Andrew!!!", "Evolution", "Tim Berne - Fractured Fairy Tales" }.Select(name => File.ReadAllLines($"\\Resources\\{name}.cue")).ToList();
        }
